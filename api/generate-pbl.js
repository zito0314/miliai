...
- 학습자는 코딩을 처음 배우는 학생이라고 가정한다. 문제는 "정답 맞히기"보다 실제 프로젝트 산출물을 조금씩 완성하는 코딩 연습이어야 한다.
- 가짜 데이터처럼 보이지 않도록 샘플 데이터의 컬럼, 행의 의미, 날짜/수량/상태/분류 같은 현실적인 기록 구조를 mission_overview와 steps에 반영한다.
- 단, 실제 군 내부 데이터나 개인정보를 요구하지 말고 비식별/가상/샘플 데이터라는 안전한 전제를 명확히 쓴다.
- 각 미션은 "문제 상황 → 데이터 또는 코드로 확인할 질문 → 작은 코딩 활동 → 결과 해석 → 산출물 개선" 흐름으로 이어져야 한다.
- 문제 유형은 단순 4지선다 반복을 피하고, 컬럼 선택, 코드 빈칸, 오류 원인 찾기, 실행 결과 예측, 분석 순서 배열, 체크리스트, AI 교관 질문, 동료 리뷰를 섞는다.
...
- 학습자는 코딩을 처음 배우는 학생이라고 가정한다. 각 step은 초보자가 "왜 이 코드를 쓰는지" 이해하고 다음 산출물로 넘어가게 만들어야 한다.
- mission_overview는 학생에게 Step 전에 노출되는 "미션 단위 문제 상황"이다. 단순 소개가 아니라 어떤 군 실무 문제가 발생했고 왜 해결해야 하는지 2~3문장으로 쓴다.
- mission_overview에는 사용할 샘플 데이터의 현실적인 단서도 포함한다. 예: 컬럼명, 기록 단위, 날짜/상태/수량/분류 값, 분석 제약.
...
- steps는 반드시 앞뒤가 연결되어야 한다. 이전 step의 선택/코드/해석이 다음 step의 판단 근거가 되게 작성한다.
- 선택형 문제도 프로젝트 맥락에 묶는다. "정의 암기"가 아니라 주어진 샘플 데이터/코드/상황에서 무엇을 해야 하는지 묻는다.
- 코드 문제는 초보자용으로 짧고 실행 가능한 조각을 사용한다. 코드에는 실제 컬럼명과 샘플 값이 드러나야 한다.
...
학습자는 코딩을 처음 배우는 군 장병이라고 가정한다.
문제는 프로젝트와 분리된 퀴즈가 아니라, 실제 PBL 산출물을 만들기 위한 작은 의사결정이어야 한다.

좋은 문제의 형태:

* "이 데이터에서 휴가 신청 쏠림을 보려면 어떤 컬럼을 먼저 확인해야 하나요?"
* "아래 pandas 코드의 빈칸에 들어갈 함수는 무엇인가요?"
* "이 코드가 실행되면 어떤 표가 만들어지나요?"
* "결측치가 있는 행을 그대로 두면 분석 결과에 어떤 문제가 생기나요?"
* "분석 순서를 데이터 불러오기 → 날짜 변환 → 집계 → 시각화 → 해석 순으로 배열하세요."
* "동료가 제출한 그래프에서 축 이름과 단위가 빠졌다면 어떤 피드백을 줘야 하나요?"

피해야 할 문제의 형태:

* 프로젝트 맥락 없이 "Python은 무엇인가요?"처럼 일반 지식만 묻는 문제
* 실제 데이터 구조가 없는 추상적인 4지선다 반복
* 학생이 아직 배우지 않은 긴 코드를 통째로 작성하게 하는 문제
* "AI가 대신 보고서를 작성하게 하세요"처럼 학습자의 사고와 코딩 활동을 건너뛰는 문제
* 실제 부대 내부 데이터, 개인 신상, 보안 정보를 입력하게 하는 문제

각 미션에는 가능한 한 서로 다른 문제 유형을 섞는다.
권장 조합:

* M1: situation_card + multiple_choice(컬럼/문제 기준 선택) + single_choice(기초 코드 선택) + self_checklist
* M2: code_fill_blank + sequence_order + code_error_finding 또는 result_prediction + pc_verification
* M3: ai_tutor_question + peer_review_request + submission
...
* 프로젝트는 "처음 코딩을 배우는 학생이 실제 문제를 해결한다"는 흐름이어야 한다. 개념 설명보다 데이터/코드/판단/산출물의 연결을 우선한다.
* 샘플 데이터가 필요한 프로젝트는 데이터셋 설명을 구체화한다. 예: 파일명, 컬럼명, 각 행이 의미하는 기록, 사용할 수 없는 민감정보, 분석 가능한 질문.
* 가짜 느낌을 줄이기 위해 "데이터가 있다"라고만 쓰지 않는다. 어떤 기록을 어떤 코드로 어떻게 확인할지 step에 드러낸다.
...
* 문제 유형은 미션 안에서 반복되지 않게 분산한다. 같은 미션에 single_choice만 여러 개 만들지 않는다.
* 모든 문제는 해당 미션 산출물의 일부를 완성하거나 검토하는 역할을 해야 한다.
...
* 문제들이 프로젝트 산출물과 직접 연결되어 있는가
* 코딩 초보자가 작은 단계로 따라갈 수 있게 구성되어 있는가
* 샘플 데이터가 현실적인 컬럼/기록 구조를 가지고 있는가
* 4지선다만 반복하지 않고 다양한 문제 유형을 섞었는가
